...
The application supports configurable logging with the following environment variables:

- `ENABLE_FILE_LOGGING`: Enable or disable file logging (default: `true`)
- `LOG_DIRECTORY`: Directory where log files are stored (default: `./logs`)
- `LOG_RETENTION_DAYS`: Number of days to retain log files (default: `3`)
- `DEBUG_LEVEL`: Set to `DEBUG` for detailed logging or `INFO` for standard logging
...
LOG_DIRECTORY=/custom/logs/path
...
When running in Docker, logs are stored in `/app/logs` inside the container and mounted to your host system via docker-compose volume mapping.
